...
{
    const tareas = document.querySelectorAll("li");
    tareas.forEach(tarea => {tarea.style.display = "flex";})
}
...
//buscar tarea especifica 
function buscarTarea()
{
    const tareas = document.querySelectorAll("li");
    const TareaABuscar = document.getElementById("buscarTarea").value;
    //Se concatena el string eliminar porque el elemento li tiene al final de la cadena el string eliminar y nunca podria ser igual a la busqueda.
    const buscar = TareaABuscar + "Eliminar";
    tareas.forEach(tarea => { 
        if (buscar === tarea.textContent) 
        {
        tarea.style.display = "flex";
        }
        else
...
document.getElementById("buscarTarea").addEventListener("click", buscarTarea);